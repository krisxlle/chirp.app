...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Stripe subscription route for Chirp+
  app.post('/api/create-subscription', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // First, check if we have a reusable Chirp+ price, otherwise create one
      let chirpPlusPrice;
      try {
        const prices = await stripe.prices.list({
          lookup_keys: ['chirp_plus_monthly'],
          limit: 1,
        });
        
        if (prices.data.length > 0) {
          chirpPlusPrice = prices.data[0];
        } else {
          ...
          chirpPlusPrice = await stripe.prices.create({
            currency: 'usd',
            unit_amount: 499, // $4.99 in cents
            recurring: { interval: 'month' },
            product: product.id,
            lookup_key: 'chirp_plus_monthly',
          });
        }
      } catch (error) {
        // Fallback: create basic product and price
        const product = await stripe.products.create({
          name: 'Chirp+ Premium Subscription',
          description: 'Premium features including handle changes, exclusive badge, and premium AI models',
        });
    ...
    try {
      switch (event.type) {
        ...
        case 'customer.subscription.deleted':
          ...
          const cancelCustomerId = deletedSub.customer as string;
