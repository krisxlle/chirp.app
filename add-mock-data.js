// Script to add mock users and data for profile cards
...
async function addMockUsers() {
  try {
    ...
    console.log('Adding mock chirps...');
    
    // Add mock chirps
    const { error: chirpsError } = await supabase.rpc('exec_sql', {
      sql: `
    ...
    if (chirpsError) {
      console.error('Error adding chirps:', chirpsError);
      return;
    }

    console.log('Adding mock follows...');
    ...
    if (followsError) {
      console.error('Error adding follows:', followsError);
      return;
    }

    console.log('✅ Mock users, chirps, and follows added successfully!');
  } catch (error) {
